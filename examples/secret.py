...
def create_overwrites(ctx, *objects):
    ...
    A `pydiscord.PermissionOverwrite` allows you to determine the permissions
    of an object, whether it be a `pydiscord.Role` or a `pydiscord.Member`.
    ...
    # for each `pydiscord.Role` or `pydiscord.Member`.
    overwrites = {
        obj: pydiscord.PermissionOverwrite(view_channel=True)
        for obj in objects
    }

    # prevents the default role (@everyone) from viewing the channel
    # if it isn't already allowed to view the channel.
    overwrites.setdefault(ctx.guild.default_role, pydiscord.PermissionOverwrite(view_channel=False))

    # makes sure the client is always allowed to view the channel.
    overwrites[ctx.guild.me] = pydiscord.PermissionOverwrite(view_channel=True)
...
@secret.command()
@commands.guild_only()
async def text(ctx: commands.Context, name: str, *objects: typing.Union[pydiscord.Role, pydiscord.Member]):
...
@secret.command()
@commands.guild_only()
async def voice(ctx: commands.Context, name: str, *objects: typing.Union[pydiscord.Role, pydiscord.Member]):
...
@secret.command()
@commands.guild_only()
async def emoji(ctx: commands.Context, emoji: pydiscord.PartialEmoji, *roles: pydiscord.Role):
